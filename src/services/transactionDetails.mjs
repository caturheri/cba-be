...
export const createTransactionDetail = async (data) => {
    ...
    const invoiceData = {
        ...
        billingAddress: primaryCustomer.originalAddress,
...
export const handleMidtransNotification = async (notification) => {
    try {
        const statusResponse = await snap.transaction.status(notification.order_id);
        console.log("Midtrans Status Response:", JSON.stringify(statusResponse, null, 2));

        if (!statusResponse) throw new Error("Invalid status response from Midtrans");

        const { order_id, transaction_status } = statusResponse;

        const transaction = await TransactionDetails.findOne({ where: { invoiceNumber: order_id } });
        if (!transaction) throw new Error("Transaction not found");

        let newStatus = "Pending";
        if (transaction_status === "settlement") newStatus = "Paid";
        else if (transaction_status === "cancel" || transaction_status === "deny" || transaction_status === "expire") newStatus = "Failed";
