  ...
  try {
    ...
    const performance = {
      ...
      customers: agent.customers.map((customer) => {
        ...
        const totalSpending = customer.transactionCustomers.reduce(
          (sum, trxCustomer) => {
        ...
        return {
          ...
          transaction: customer.transactionCustomers.map(
            (trxCustomer) => trxCustomer.transactionDetail
          ), // Akses transactionDetails melalui TransactionCustomers
          totalTransactions: customer.transactionCustomers.length,
          totalSpending,
        };
      }),
      totalCustomers: agent.customers.length,
      totalSpending: agent.customers.reduce(
        (sum, customer) =>
          sum +
          customer.transactionCustomers.reduce((sum, trxCustomer) => {
            const trx = trxCustomer.transactionDetail;
            return sum + trx.subtotal * trx.quantity;
          }, 0),
        0
